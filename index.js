...
function addGamesToPage(games) {
    ...
    for (let g in games) {
        ...
        gameCard.innerHTML = `
            <img class="game-img" src=${game.img} alt="Image" />
            <h3>${game.name}</h3>
...
const totalIndividualContributions = GAMES_JSON.reduce((accumulator, currentValue) => accumulator + currentValue.backers, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `
  <p>$${totalIndividualContributions.toLocaleString('en-US')}</p>
`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const totalAmountPledged = GAMES_JSON.reduce((accumulator, currentValue) => accumulator + currentValue.pledged, 0);
...
let numUnfunded = GAMES_JSON.reduce((acc, current) => {
    if (current.goal > current.pledged) {
        return acc + 1;
    }
    return acc;
}, 0);
...
let tempStr = `A total of $${totalAmountPledged.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games.
                Currently, ${numUnfunded} ${numUnfunded != 1 ? "games remain" : "game remains"} unfunded.
                We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const desc = document.getElementById("description-container");
let newDesc = document.createElement("p");
newDesc.innerHTML = tempStr;
